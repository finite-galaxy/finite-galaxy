...
namespace {
  ...
  {
    static const Command cancellers(Command::LAND | Command::JUMP | Command::BOARD | Command::AFTERBURNER
...
void AI::UpdateKeys(PlayerInfo &player, Command &activeCommands)
{
  ...
  {
    bool cancelled = (autoPilot.Has(Command::JUMP) && !activeCommands.Has(Command::JUMP));
    cancelled |= (autoPilot.Has(Command::STOP) && !activeCommands.Has(Command::STOP));
    cancelled |= (autoPilot.Has(Command::LAND) && !activeCommands.Has(Command::LAND));
    cancelled |= (autoPilot.Has(Command::BOARD) && !activeCommands.Has(Command::BOARD));
    if(cancelled)
...
void AI::MoveIndependent(Ship &ship, Command &command) const
{
  ...
  if(!ship.GetTargetSystem() && !ship.GetTargetStellar() && !shouldStay)
  {
    ...
    const set<const System *> &links = ship.Attributes().Get("jump drive")
      ? origin->Neighbours() : origin->Links();
    if(jumps)
    {
...
void AI::DoSurveillance(Ship &ship, Command &command, shared_ptr<Ship> &target) const
{
  ...
  else
  {
    ...
    {
      const auto &links  = ship.Attributes().Get("jump drive") ? system->Neighbours() : system->Links();
    ...
    if(hardpoint.CanAim())
    {
      ...
      for(const Body *target : targets)
      {
        ...
        if(!weapon->Acceleration())
          v -= ship.Velocity();
        // By the time this action is performed, the targets will have
        // moved forward one time step.
        p += v;
  ...
  else if(activeCommands.Has(Command::JUMP))
  {
    if(!ship.GetTargetSystem() && !isWormhole)
    {
      ...
      const auto &links = (ship.Attributes().Get("jump drive") ?
        ship.GetSystem()->Neighbours() : ship.GetSystem()->Links());
      for(const System *link : links)
      {
