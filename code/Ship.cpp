...
void Ship::FinishLoading(bool isNewInstance)
{
  ...
  string warning;
  for(const string &attr : set<string>{"cargo space", "outfit space", "core space", "engine space", "weapon space"})
  {
  ...
  if(!warning.empty())
  {
    ...
    // no names. Instead, print the outfits to facilitate identification.
...
bool Ship::CanRefuel(const Ship &other) const
{
  ...
  return (fuel - JumpFuel(targetSystem) >= other.JumpFuelMissing()) && (!other.GetGovernment()->IsEscort() || !other.GetGovernment()->IsPlayer() || government->GetFuelPrice() <= Preferences::GetMaxPrice());
}
...
double Ship::JumpFuel(const System *destination) const
{
  ...
  if(attributes.Get("jump drive") && currentSystem->Neighbours().count(destination))
